...
const LETTER = [
  'خرگوش کوچولوم... 🐰🤍 یه چیزی هست که خیلی دلم می‌خواد همیشه یادت بمونه...',
  'دلم می‌خوام هر وقت کنار منی، دلت آروم باشه. از اون آرامشایی که آدم حتی دلیلش رو هم نمی‌دونه؛ فقط می‌فهمه حال دلش خوبه و خیالش راحته. ☁️',
  'دختر نازم، هر حسی که داری، چه قشنگ‌تریناش و چه سخت‌تریناش، پیش من جاش امنه. دوست دارم هر وقت دلت خواست حرف بزنی، با خیال راحت حرف بزنی، هر وقت دلت خواست بخندی، از ته دل بخندی و حتی اگه فقط دلت سکوت خواست، همون سکوت هم بینمون قشنگ باشه. 🩷',
  'ذوقت، شیطونیتات و حتی خوشحال شدنت برای چیزای کوچیک، برای من از قشنگ‌ترین چیزای دنیاست... و بدون که لبخندت برای علیرضا، واقعاً دنیا رو قشنگ‌تر می‌کنه. 🥹',
  'راستش، قشنگ‌ترین تصویری که از خودمون توی ذهنم دارم، اینه که هر وقت اسم همدیگه میاد، ناخودآگاه لبخند بزنیم و ته دلمون بگیم کنارش حالم خوبه...» ✨',
  'رابطه‌ای که بین ما شکل گرفته، برای من فقط با هم حرف زدن یا کنار هم بودن نیست. دوست دارم جایی باشه که هر دومون توش حال دلمون خوب باشه؛ جایی که کنار هم احساس آرامش کنیم، به هم اعتماد داشته باشیم و بدونیم هر اتفاقی بیفته، همیشه می‌تونیم روی هم حساب کنیم. 🤍',
  'نمی‌خوام رابطه‌مون فقط شروع قشنگی داشته باشه؛ دوست دارم هر روز قشنگ‌تر از دیروز بشه. از اون رابطه‌هایی که قشنگیش فقط توی حرف‌های قشنگ نیست، توی توجه‌های کوچیکه، توی احترام، توی شنیدن همدیگه، توی کنار هم موندن و ساختن روزهایی که بعداً با لبخند یادشون می‌کنیم. 💞',
  'دلم می‌خوام کم‌کم برای هم تبدیل به همون آدمی بشیم که توی شلوغ‌ترین روزها هم فکر کردن بهش، حال دلمون رو بهتر می‌کنه. از اون رابطه‌هایی که آدم توش احساس امنیت می‌کنه، نه نگرانی؛ احساس آرامش می‌کنه، نه استرس. 🫂',
  'دلم می‌خوام هر وقت روزت شلوغ شد یا ذهنت پر از فکر بود، کنار من یه نفس راحت بکشی. نه اینکه همه‌ی مشکل‌ها از بین برن، فقط دلت سبک‌تر بشه و حس کنی یه نفر هست که بودنت براش مهمه؛ حتی وقتی هیچ حرفی برای گفتن نداری. 🌙',
  'برای من قشنگ‌ترین اتفاق اینه که کم‌کم کنار هم تبدیل به همون آدمی بشیم که هر وقت یه اتفاق خوب براش می‌افته، یا دلش یه ذره می‌گیره، اولین کسی که دلش می‌خواد کنارش باشه، اون یکیه. 💫',
  'همین... همین حس برای من از هر چیزی قشنگ‌تره. دلم می‌خوام هر بار که کنار منی، قلبت یه ذره آروم‌تر از قبل بتپه و ته دلت مطمئن باشی که پیش من همیشه یه جای امن برای خندیدن، حرف زدن، سکوت کردن و بدون هیچ نگرانی، فقط خودت بودن هست. 💗',
];
...
const NO_MESSAGES = [
  ...
  'هنوزم نه؟ ',
  'بیشترتر بهم برخورد',
  'بیشترترترتر بهم برخورد😂',
  'خب... بازم نه؟ 🤨',
  'خب نمیتونیی بهت بر خورد😂',
];
...
const Server = {
  ...
  /** ارسال بی‌صدا — اگر خطا باشد، یک‌بار توست کوچک نمایش داده می‌شود */
  push(record) {
    ...
    fetch('api/save.php', {
      ...
      .then(async (res) => {
        ...
        if (!res.ok || data.ok !== true) {
          console.error('[server] save failed:', res.status, data);
          this.notifyFail();
        }
      })
      .catch((err) => {
  ...
  /** نمایش یک‌بارِ پیام هماهنگ‌سازی ناموفق (بدون توقف کاربر) */
  notifyFail() {
    if (this._toastShown) return;
    this._toastShown = true;
    const toast = document.createElement('div');
    toast.className = 'sync-toast';
    toast.textContent = 'هماهنگ‌سازی پاسخ‌ها انجام نشد 😢';
    ...
    if (q.type === 'choice' || q.type === 'yesno') {
      ...
      form.addEventListener('submit', (e) => {
        ...
        if (!value) {
          ...
          const err = $('.field-error', form);
          if (!err) {
            const el = document.createElement('p');
            el.className = 'field-error';
            form.appendChild(el);
          }
          $('.field-error', form).textContent = 'بهم برخورد...';
...
async function typeLetter() {
  ...
  sign.innerHTML = '<div class="sign-line"></div><p class="sign-text">   از طرف یه فرفری به بچش</p>';
